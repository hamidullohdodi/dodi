...
const (
	signingKey = "Secret key for Artisan Connect service"
)
...
func GenereteJWTToken(user_id string, username string) (*pb.Token, error) {
	...
	expiresIn := time.Now().Add(3600 * time.Second).Unix()
	...
	if err != nil {
		return nil, errors.New("error while generating access token: " + err.Error())
	}
	...
	rftclaims["exp"] = time.Now().Add(7200 * time.Second).Unix() // Set refresh token expiry separately if needed
	refresh, err := refreshToken.SignedString([]byte(signingKey))
	if err != nil {
		return nil, errors.New("error while generating refresh token: " + err.Error())
	}
